			...
			return $this->password;
					...
					**/
					...
					function getSelectQuery(){
						return "SELECT `userid`, `password`, `email` FROM `registration` WHERE `email`=? AND `user_type`=?";
					}
						...
						//checking if data entered is empty or not.
							...
							else
							{
								...
								if (!filter_var($emailVal, FILTER_VALIDATE_EMAIL)) {
									...
									if($sql =$con->prepare($this->getSelectQuery())){
										// Bind the appropriate values that have to be inserted into db
										$sql->bind_param("ss", $emailVal,$userType);
